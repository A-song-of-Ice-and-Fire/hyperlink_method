...
class Jaccard(Indicator):
    ...
    def forward(self, edge_matrix: Union[np.ndarray, ssp.spmatrix]) -> Optional[List[float]]:
        ...
        for edge_index in range(edge_matrix.shape[1]):
            ...
            intersection_vector = np.logical_and(left_node_matrix,right_node_matrix).sum(axis=1)
            union_vector = np.logical_or(left_node_matrix,right_node_matrix).sum(axis=1)
            jc_vector = intersection_vector / union_vector
            jc_vector[np.isnan(jc_vector)] = 0                      
            prediction.append(
                jc_vector.mean()
            )              
        return  prediction